...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    ...
    : hash_(hash), kequal_(kequal), prober_(prober), totalProbes_(0),
      resizeAlpha_(resizeAlpha), mIndex_(0), size_(0)
{
...
void HashTable<K, V, Prober, Hash, KEqual>::insert(const ItemType& p)
{
    ...
    double loadFactor = static_cast<double>(size_) / CAPACITIES[mIndex_];
    if (loadFactor >= resizeAlpha_) {
        resize();
    }

    // Probe to find index for this key
    HASH_INDEX_T loc = probe(p.first);

    if (loc == npos) {
        throw std::logic_error("No available slot for insertion.");
    }
    ...
    if (table_[loc] != nullptr && !table_[loc]->deleted) {
        ...
        // New key or overwriting a deleted one
...
void HashTable<K, V, Prober, Hash, KEqual>::remove(const KeyType& key)
{
    ...
    if (loc == npos || table_[loc] == nullptr || table_[loc]->deleted) {
        // Key not found or already marked deleted
        return;
    }

    table_[loc]->deleted = true;
    --size_; // Only decrement size on actual deletion
}
...
void HashTable<K, V, Prober, Hash, KEqual>::resize()
{
    if (mIndex_ + 1 >= sizeof(CAPACITIES) / sizeof(HASH_INDEX_T)) {
        throw std::logic_error("No more sizes available to resize.");
    }
    ...
    // Rehash all valid (non-deleted) items
    for (HashItem* item : oldTable) {
        if (item != nullptr && !item->deleted) {
            insert(item->item); // Reinsert into new table
        }
